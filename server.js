...
app.get('/order', async (request, response) => {
    ...
    response.render('order', {
        ...
        pageOrder: true,
        produit: await getProduit(),
        order: await joinPanierMenu(request.user.id_utilisateur),
        user: request.user,
        admin: request.user && request.user.id_type_utilisateur === 2
    });
});
...
app.route('/api/order')
    .get(async (request, response) => {
        ...
        // Lister le panier
        let order = await getOrder(request.user.id_utilisateur);
    ...
    .post(async (request, response) => {
        ...
        // Ajouter un article
        await addItem(request.user.id_utilisateur,request.body.id_produit);
    ...
    .delete(async (request, response) => {
        ...
        // Vider le panier
        await clearOrder(request.user.id_utilisateur);
    ...
    .patch(async (request, response) => {
        ...
        await reduireQuantiteItem(request.user.id_utilisateur,request.body.id_produit);
        response.sendStatus(200);
    });
...
app.delete('/api/itemOrder', async (request, response) => {
    if(!request.user){
     ...
     // retire un article
     await retirerItem(request.user.id_utilisateur,request.body.id_produit);
     response.sendStatus(200);
});
app.patch('/api/itemOrder', async (request, response) => {
    ...
    await incrItem(request.user.id_utilisateur,request.body.id_produit);
    response.sendStatus(200);
});
...
app.get('/api/menuPanier',async (request, response) => {
    // Lister le menu
    let order = await joinPanierMenu(request.user.id_utilisateur);
    response.status(200).json(order);
})
...
app.get('/api/beauPanier', async (request, response) => {
    ...
    // Liste panier sauvegarder
    let beauPanier = await beauJoinPanierMenu(request.user.id_utilisateur);
